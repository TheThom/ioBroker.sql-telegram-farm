		...
		try {
			...
			switch (sqlFunction) {
				...
				case MYSQL.GET.MACHINES.MAINTENANCE.FREE_ID: {
					...
					console.log(machineId);
					const [maintenanceId] = await mySqlCon.query(
						'SELECT (maintenanceId + 1) AS maintenanceId FROM machine_maintenance_plan WHERE machineId="' +
							machineId +
				...
				case MYSQL.GET.MACHINES.HISTORY: {
					...
					text.push(' ');
					if (hourMeterOffset[0].hourMeterOffset != 0) {
						text.push('Offset Betriebsstundenzähler: <b>' + hourMeterOffset[0].hourMeterOffset + 'h</b>');
					}
					text.push('<code>');
					...
					text.push(
						'Stunden gesamt    : </code><span class="tg-spoiler">' +
							(parseFloat(resultStat[0].durationTotal) + parseFloat(hourMeterOffset[0].hourMeterOffset)) +
							'h</span><code>',
					);
					text.push(
