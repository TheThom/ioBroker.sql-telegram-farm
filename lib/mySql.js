...
class mySql {
		...
		try {
			...
			switch (sqlFunction) {
				...
				case MYSQL.SET.MACHINES.ADD_USE: {
					...
					const [resultHoursUse] = await mySqlCon.query(
						'SELECT SUM(hoursUse) AS durationTotal FROM machine_use WHERE machineId="' + machineId + '"',
					);
					let additionallyUsedHours = parseFloat(parameters[MENU.MACHINES_CATEGORY.MACHINE.ACTIONS_USE._]);
					this.adapter.log.error(String(additionallyUsedHours));

					if (hourMeterOffset > 0) {
						additionallyUsedHours =
							parseFloat(parameters[MENU.MACHINES_CATEGORY.MACHINE.ACTIONS_USE._]) -
							(parseFloat(resultHoursUse[0].durationTotal) + parseFloat(hourMeterOffset));
						this.adapter.log.error(String(additionallyUsedHours));
						if (additionallyUsedHours < 0.0 || additionallyUsedHours > 999.0) {
							this.sendTextToUser(
								user,
								'konnte nicht übernommen werden. Gültige Werte: 0 - 999h zusätzlich',
							);
							this.sendTextToUser(
								user,
								'Der Stundenzähler hat bereits ' +
									String(parseFloat(resultHoursUse[0].durationTotal) + parseFloat(hourMeterOffset)) +
									'h',
							);
							return false;
						}
						...
						if (additionallyUsedHours < 0.0 || additionallyUsedHours > 999.0) {
							this.sendTextToUser(user, 'konnte nicht übernommen werden. Gültige Werte: 0 - 999h');
							return false;
						}
					}
				...
				case MYSQL.SET.RECEIPT.ADD_MAPPING: {
					const sql = 'INSERT INTO receipt_item_mappings(receiptId, itemId, entityType, entityId) ';
					const arrValues = [
						String(parameters[MYSQL.SET.RECEIPT.ADD_MAPPING.receiptId]),
						String(parameters[MYSQL.SET.RECEIPT.ADD_MAPPING.itemId]),
						String(parameters[MYSQL.SET.RECEIPT.ADD_MAPPING.entityType]),
						String(parameters[MYSQL.SET.RECEIPT.ADD_MAPPING.entityId]),
					];
					const values = generateSqlValues(arrValues);
					const [result] = await mySqlCon.query(sql + values);
					if (result.warningStatus == '') {
					...
					await this.sendTextToUser(
						user,
						'MYSQL.SET.RECEIPT.ADD_MAPPING: warningStatus:' + result.warningStatus,
					);
					return false;
				}

				case MYSQL.SET.RECEIPT.SAVE_DOCREC: {
					const sql = 'INSERT INTO receipt(storeId, total, dateTime, isAssigned, note)';
					const arrValues = [
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.ID._]),
						String(parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['total'].toFixed(2)),
						String(parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['datetime']),
						parameters[MENU.DIALOG.DOCREC.CATEGORY._] === MENU.DIALOG.DOCREC.CATEGORY.NO_CATEGORY
							? String(0)
							: String(1),
						String(parameters[MENU.DIALOG.DOCREC.FILE_NOTE._]),
					];
					const values = generateSqlValues(arrValues);
					const [result] = await mySqlCon.query(sql + values);
					if (result.warningStatus == '') {
						const [receiptIdArr] = await mySqlCon.query('SELECT LAST_INSERT_ID() AS receiptId');
						const receiptId = receiptIdArr[0].receiptId;
						const sql2 = 'INSERT INTO receipt_items(receipt, item, name, quantity, price, itemId)';
						const arrValues2 = [];
						for (const item in parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items']) {
							arrValues2.push([
								String(receiptId),
								String(parseInt(item) + 1),
								String(parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['name']),
								String(
									parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['quantity']
										? parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['quantity']
										: 0,
								),
								String(
									parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['price']
										? parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['price'].toFixed(2)
										: 0,
								),
								String(parameters[MENU.DIALOG.DOCREC.SHOW_REC._]['items'][item]['item_id']),
							]);
						}

						const values2 = generateSqlValues(arrValues2);
						const [result] = await mySqlCon.query(sql2 + values2);
						if (result.warningStatus == '') {
							if (parameters[MENU.DIALOG.DOCREC.CATEGORY._] !== MENU.DIALOG.DOCREC.CATEGORY.NO_CATEGORY) {
								const param = {};
								param[MYSQL.SET.RECEIPT.ADD_MAPPING.receiptId] = receiptId;
								param[MYSQL.SET.RECEIPT.ADD_MAPPING.itemId] = '0';
								param[MYSQL.SET.RECEIPT.ADD_MAPPING.entityType] = await this.getEnumId(
									user,
									MYSQL.ENUM.TYPES.RECIEPT.MAPPING.ENTITY_TYPE,
									parameters[MENU.DIALOG.DOCREC.CATEGORY._],
								);
								param[MYSQL.SET.RECEIPT.ADD_MAPPING.entityId] = '0';
								const res = await this.set(user, MYSQL.SET.RECEIPT.ADD_MAPPING, param);
								if (!res) {
									return false;
								}
							}
							...
							await this.sendTextToUser(
								user,
								'MYSQL.SET.RECEIPT.SAVE_DOCREC: warningStatus:' + result.warningStatus,
							);
						}
					}
					return false;
				...
				//#region MASTER_DATA
				case MYSQL.SET.MASTER_DATA.SAVE_ACCOUNT: {
					if (parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.ID._] == 0) {
						const [res] = await mySqlCon.query('SELECT MAX(accountId) + 1 AS newId FROM accounts');
						parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.ID._] = String(res[0].newId);
					}
					const sql =
						'REPLACE INTO accounts (accountId, name, category, note, street, houseNumber, postalCode, city, state, countryCode, created)';
					const arrValues = [
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.ID._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.NAME._]),
						String(
							await this.getEnumId(
								user,
								MYSQL.ENUM.TYPES.ACCOUNTS.CATEGORY,
								parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.CATEGORY._],
							),
						),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.NOTE._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.STREET._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.HOUSENUMBER._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.POSTALCODE._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.CITY._]),
						String(parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.STATE._]),
						String(
							await this.getEnumId(
								user,
								MYSQL.ENUM.TYPES.ACCOUNTS.COUNTRY_CODES,
								parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.COUNTRYCODE._],
							),
						),
						createDateTime(),
					];
	...
	async get(user, sqlFunction, parameters) {
		try {
			await this.mySqlEnsureConnection(user);
			switch (sqlFunction) {
				case MYSQL.ENUM.VALID_TEXTS._: {
					const [arrEnum] = await mySqlCon.query(
						'SELECT id, text FROM enum WHERE type = "' + parameters[MYSQL.ENUM.VALID_TEXTS.P_TYPE] + '"',
					);
					const result = [];
					for (const count in arrEnum) {
						result.push(String(arrEnum[count].text));
					}
					return result;
				}
				case MYSQL.ENUM.ID._: {
					const [arrEnum] = await mySqlCon.query(
						'SELECT id FROM enum WHERE type = "' +
							parameters[MYSQL.ENUM.ID.P_TYPE] +
							'" AND text = "' +
							parameters[MYSQL.ENUM.ID.P_TEXT] +
							'"',
					);
					const result = String(arrEnum[0].id);
					return result;
				}
				//#region FIREWOOD
				...
				case MYSQL.GET.FIREWOOD.DATASET_BY_NUMBER: {
					const [arrDATASET] = await mySqlCon.query(
						'SELECT id, nr, activ, amount, typ, DATE_FORMAT(dateMod, "%Y-%m-%d") as dateMod, humidity, notes, location FROM wood WHERE activ = "2" AND nr = "' +
							parameters[MENU.FIREWOOD.EDIT.NUMBER._] +
							'"',
					);
					console.warn(arrDATASET);
					parameters[MENU.FIREWOOD.EDIT.ID._] = arrDATASET[0].id;
					parameters[MENU.FIREWOOD.EDIT.NUMBER._] = arrDATASET[0].nr;
					parameters[MENU.FIREWOOD.EDIT.NUMBER.CHANGE] = arrDATASET[0].nr;
					parameters[MENU.FIREWOOD.EDIT.AMOUNT_DETAILED._] = arrDATASET[0].amount;
					parameters[MENU.FIREWOOD.EDIT.TYPE._] = arrDATASET[0].typ;
					parameters[MENU.FIREWOOD.EDIT.DATE] = arrDATASET[0].dateMod;
					parameters[MENU.FIREWOOD.EDIT.HUMIDITY._] = arrDATASET[0].humidity;
					parameters[MENU.FIREWOOD.EDIT.NOTES._] = arrDATASET[0].notes;
					parameters[MENU.FIREWOOD.EDIT.LOCATION._] = arrDATASET[0].location;
					return parameters;
				}
				...
				//#region MASTER_DATA
				/*
				case MYSQL.GET.MASTER_DATA.ACCOUNTS.DATASET_BY_ID: {
					const [arrDATASET] = await mySqlCon.query(
						'SELECT accountId, name, category, note, street, houseNumber, postalCode, city, state, countryCode, FROM accounts WHERE accountId = "' +
							parameters +
							'"',
					);
					const result = {};
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.ID._] = arrDATASET[0].accountId;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.NAME._] = arrDATASET[0].name;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.CATEGORY._] = arrDATASET[0].category;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.NOTE._] = arrDATASET[0].note;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.STREET._] = arrDATASET[0].street;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.HOUSENUMBER._] = arrDATASET[0].houseNumber;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.POSTALCODE._] = arrDATASET[0].postalCode;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.CITY._] = arrDATASET[0].city;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.STATE._] = arrDATASET[0].state;
					result[MENU.MASTER_DATA.ACCOUNT.EDIT.COUNTRYCODE._] = arrDATASET[0].countryCode;

					return result;
				}*/

				case MYSQL.GET.MASTER_DATA.ACCOUNTS.EXISTS: {
					const [arrDATASET] = await mySqlCon.query(
						'SELECT IFNULL(MIN(accountId), 0) AS accountId FROM accounts WHERE name="' +
							parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.NAME._] +
							'" AND street="' +
							parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.STREET._] +
							'" AND houseNumber="' +
							parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.HOUSENUMBER._] +
							'" AND postalCode="' +
							parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.POSTALCODE._] +
							'" AND city="' +
							parameters[MENU.MASTER_DATA.ACCOUNT.EDIT.CITY._] +
							'"',
					);
					return arrDATASET[0].accountId;
				}
	...
	async getEnumValidTexts(user, enumType) {
		const param = [];
		param[MYSQL.ENUM.VALID_TEXTS.P_TYPE] = enumType;
		const texts = this.get(user, MYSQL.ENUM.VALID_TEXTS._, param);
		return texts;
	}

	async getEnumId(user, enumType, enumText) {
		const param = [];
		param[MYSQL.ENUM.ID.P_TYPE] = enumType;
		param[MYSQL.ENUM.ID.P_TEXT] = enumText;
		const id = this.get(user, MYSQL.ENUM.ID._, param);
		return id;
	}
...
function generateSqlValues(arrValues) {
	let result = ' VALUES';
	if (arrValues[0].constructor === Array) {
		//Multiple sets of values
		for (const row in arrValues) {
			result = result + '(';
			for (const column in arrValues[row]) {
				result = result + "'" + String(arrValues[row][column]).replace(/'/g, '`') + "', ";
			}
		...
		result = result + '(';
		for (const column in arrValues) {
			result = result + "'" + String(arrValues[column]).replace(/'/g, '`') + "', ";
		}
		result = result?.substring(0, result.length - 2) + ')';
	}
